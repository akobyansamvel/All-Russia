...
export default {
	...
	methods: {
		async fetchData() {
			try {
				const mainResponse = await axios.get('http://127.0.0.1:5000/data_main_page')
				this.mainArticle = mainResponse.data.main_article[0]
				this.similarArticles = mainResponse.data.same_as_main.slice(0, 3)

				const economicsResponse = await axios.get('http://127.0.0.1:5000/data_news_economics')
				this.economics = economicsResponse.data

				const politicsResponse = await axios.get('http://127.0.0.1:5000/data_news_politics')
